...
__global__ void total(float * input, float * output, int len) {
    ...
    //@@ correct index
	__shared__ float num[BLOCK_SIZE*2];
	int idx   = threadIdx.x;
	int start = blockIdx.x*BLOCK_SIZE*2;
	
	if (start+idx<len) 
		num[idx] = input[start+idx];
	else
		num[idx] = 0;
	if (start+idx+BLOCK_SIZE<len) 
		num[idx+BLOCK_SIZE] = input[start+idx+BLOCK_SIZE];
	else
		num[idx+BLOCK_SIZE] = 0;
	
	__syncthreads();
	
	for (int stride = BLOCK_SIZE;stride>=1;stride/=2){
		if (idx<stride){
			num[idx] += num[idx+stride];
		}
		__syncthreads();
	}
	if (idx == 0)
		output[blockIdx.x] = num[0];
}
